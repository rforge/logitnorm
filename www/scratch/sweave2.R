## tidy.source() is currently a function in the package 'animation' 
## to tidy up R source code with comments being partially preserved 
## so that your code looks more tidy and comments will not be discarded 
## see ?tidy.source after library(animation) 
`tidy.source` <-function(source = "clipboard", keep.comment = TRUE, 
	...
	if (keep.comment) { 
		...
		while (length(grep(sprintf("%s|%s", begin.comment, end.comment), text.lines))) { begin.comment = identifier() 
			end.comment = identifier() 
		} 
...
## modified version of parse and deparse 
...
## Sweave function makeRweaveLatexCodeRunner() modified to use parse2() and 
## deparse2() to preserve comments 
RweaveCodeRunner2 <-function(evalFunc = RweaveEvalWithOpt) { 
	...
	RweaveLatexRuncode <-function(object, chunk, options) { 
		...
		if (options$split) {
			...
			if (is.null(chunkout)) {
				...
				if (!is.null(options$label)) object$chunkout[[chunkprefix]] <-chunkout
			} }
		...
		if (options$expand) 
			lastshown <-0L else lastshown <-srcline -1L 
		thisline <-0 
		...
		if (openSchunk) { cat("\\end{Schunk}\n", file = chunkout, append = TRUE) 
			linesout[thisline + 1L] <-srcline 
			thisline <-thisline + 1L 
		} 
		if (is.null(options$label) & options$split) close(chunkout) 
		if (options$split & options$include) { cat("\\input{", chunkprefix, "}\n", sep = "", file = object$output, 
				append = TRUE) 
			linesout[thisline + 1L] <-srcline 
			thisline <-thisline + 1L 
			
		} 
		if (options$fig && options$eval) { if (options$eps) { 
				grDevices::postscript(file = paste(chunkprefix, "eps", sep = "."), width = options$width, height = options$height, paper = "special", horizontal = FALSE) 
				err <-try({ 
						SweaveHooks(options, run = TRUE) 
			...
			if (options$pdf) { 
				...
				grDevices::pdf(file = paste(chunkprefix, "pdf", sep = "."), width = options$width, height = options$height, version = options$pdf.version, encoding = options$pdf.encoding) 
				err <-try({ 
						SweaveHooks(options, run = TRUE) 
						eval(chunkexps, envir = .GlobalEnv) 
			...
			if (options$png) { 
				...
				grDevices::png(file = paste(chunkprefix, "png", sep = "."), width = options$width, height = options$height) 
...
assignInNamespace("makeRweaveLatexCodeRunner", RweaveCodeRunner2, 
	ns = "utils") 
